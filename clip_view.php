<?php
    ...
    //旅行記の取得
    $sql = 'SELECT d.*, a.area_name, c.country_name FROM dialies AS d INNER JOIN areas_dialies AS ad ON d.dialy_id = ad.dialies_id INNER JOIN areas AS a ON ad.area_id = a.area_id INNER JOIN countries AS c ON a.country_id=c.country_id INNER JOIN favs as f ON d.dialy_id = f.dialy_id WHERE f.user_id = ? ';
    ...
    // CLIPデータ数の取得
    $sql = 'SELECT COUNT(*) AS cnt FROM dialies AS d INNER JOIN favs as f ON d.dialy_id = f.dialy_id WHERE f.user_id= ?';
    $data = array($user_id);
    $stmt = $dbh->prepare($sql);
    $stmt->execute($data);

    $c_clip_dialy = $stmt->fetch(PDO::FETCH_ASSOC);

    // echo '<pre>';
    // echo '$c_clip_dialy = ';
    // var_dump($c_clip_dialy);
    // echo '</pre>';
      ...
      <div class="row">
        ...
        <div class="col-sm-3">
          <div class="span3">
            ...
            <p>クリップ数： <?php echo $c_clip_dialy['cnt']; ?></p><br>
          </div>
        </div>
      </div>
    </div>
